...
void C_HookedEvents::FireGameEvent(IGameEvent* event)
{
	...
	else if (!strcmp(event_name, crypt_str("weapon_fire")))
	{
		...
		if (user == m_engine()->GetLocalPlayer())
		{
			...
			if (current_shot)if (c_config::get()->b["impacts"])
			{
	...
	else if (!strcmp(event_name, crypt_str("bullet_impact")))
	{
		...
		if (user == m_engine()->GetLocalPlayer())
		{
			...
			if ( c_config::get( )->b[ "impacts" ] )
				m_debugoverlay()->BoxOverlay(position, Vector(-2.0f, -2.0f, -2.0f), Vector(2.0f, 2.0f, 2.0f), QAngle(0.0f, 0.0f, 0.0f), 0, 0, 255, 200, 3.0f);
	...
	else if (!strcmp(event_name, crypt_str("player_hurt")))
	{
		...
		if (attacker_id == m_engine()->GetLocalPlayer() && user_id != m_engine()->GetLocalPlayer())
		{
			...
			if (current_shot)
			{
				current_shot->shot_info.result = (Color(D3DCOLOR_RGBA(255, 0, 0, 255)), crypt_str("Hit"));
	...
	else if (!strcmp(event_name, crypt_str("round_start")))
	{
		...
		//SkinChanger::precached_on_round_start = true;

		//SkinChanger::should_precache("models/player/holiday/facemasks/facemask_battlemask.mdl");
