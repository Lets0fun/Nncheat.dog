...
std::vector<int> aim::get_hitboxes(adjust_data* record, bool optimized) {
	...
	if (c_config::get()->auto_check(c_config::get()->i["rage_baim_enabled"], c_config::get()->i["rage_baim_enabled_style"])) {
		hitboxes = { HITBOX_UPPER_CHEST, HITBOX_CHEST, HITBOX_LOWER_CHEST, HITBOX_STOMACH, HITBOX_PELVIS };
		return hitboxes;
	}
	...
	if (c_config::get()->m["rage_hitbox"].at(3)) {
		hitboxes.insert(hitboxes.end(), { HITBOX_RIGHT_UPPER_ARM, HITBOX_RIGHT_FOREARM, HITBOX_RIGHT_HAND });
		hitboxes.insert(hitboxes.end(), { HITBOX_LEFT_UPPER_ARM, HITBOX_LEFT_FOREARM, HITBOX_LEFT_HAND });
	}

	if (c_config::get()->m["rage_hitbox"].at(4)) {
		hitboxes.insert(hitboxes.end(), { HITBOX_RIGHT_THIGH, HITBOX_RIGHT_CALF });
		hitboxes.insert(hitboxes.end(), { HITBOX_LEFT_THIGH, HITBOX_LEFT_CALF });
	}

	if (c_config::get()->m["rage_hitbox"].at(5)) {
		hitboxes.insert(hitboxes.end(), { HITBOX_RIGHT_FOOT, HITBOX_LEFT_FOOT });
	}
