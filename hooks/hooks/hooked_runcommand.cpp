...
void __vectorcall hooks::hooked_clmove(float accumulated_extra_samples, bool bFinalTick)
{
	if (g_ctx.globals.fakeducking)
		return ((CLMove_t)original_clmove)(accumulated_extra_samples, bFinalTick);
	...
	else if (g_ctx.globals.should_recharge)
		g_ctx.globals.should_recharge = false;
	...
	while (g_ctx.globals.shift_ticks > 0)
	{
		...
		//g_ctx.get_command()->m_buttons &= ~(IN_ATTACK | IN_ATTACK2);
