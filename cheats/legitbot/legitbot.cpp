...
void legit_bot::createmove(CUserCmd* cmd)
{
	...
	if (c_config::get()->b["misc_autowep"])
		auto_pistol(cmd);
	...
	// is enemy in smoke?
	if (!c_config::get()->b["legit_smokecheck"] && in_smoke(g_ctx.globals.eye_pos, target->hitbox_position(0)))
	{
		autofire_delay = 0;
	...
	// are we flashed?
	if (!c_config::get()->b["legit_flashcheck"] && g_ctx.local()->m_flFlashDuration() >= 0.1f)
	{
		autofire_delay = 0;
	...
	// is local in air?
	if (!c_config::get()->b["legit_aircheck"] && !(g_ctx.local()->m_fFlags() & FL_ONGROUND))
	{
		if (should_do_rcs)
			do_rcs(cmd);
