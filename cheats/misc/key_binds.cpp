...
void key_binds::update_key_bind(key_bind* key_bind, int key_bind_id)
{
	...
	case HOLD:
		...
		case 2:
			...
			if (misc::get().double_tap_key && c_config::get()->auto_check(c_config::get()->i["rage_dt_key"], c_config::get()->i["rage_dt_key_style"]) != g_cfg.antiaim.hide_shots_key.key)
		...
		case 12:
			...
			if (misc::get().hide_shots_key && c_config::get()->auto_check(c_config::get()->i["rage_dt_key"], c_config::get()->i["rage_dt_key_style"]) != g_cfg.ragebot.double_tap_key.key)
	...
	case TOGGLE:
		if (!key_bind->holding && is_button_down)
		{
			...
			case 12:
				...
				if (misc::get().hide_shots_key && g_cfg.antiaim.hide_shots_key.key != c_config::get()->auto_check(c_config::get()->i["rage_dt_key"], c_config::get()->i["rage_dt_key_style"]))
					misc::get().double_tap_key = false;
