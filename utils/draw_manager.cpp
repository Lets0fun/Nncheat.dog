...
void render::Draw3DCircle(const Vector& origin, float radius, Color color, int line_thinkness)
{
...
void render::Draw3DFilledCircleTest(const Vector& origin, float radius, Color color)
{
	...
	static auto step = M_PI * 2.0f / 18.0f;
...
void render::Draw3DRainbowCircle(const Vector& origin, float radius, int line_thinkness)
{
...
void FilledQuad( const Vector2D& p1, const Vector2D& p2, const Vector2D& p3, const Vector2D& p4, const Color& color ) {
	...
	m_surface()->DrawSetColor( color );
	m_surface()->DrawTexturedPolygon( 4, vertices );
}

void render::DrawArc( const Vector2D& center, float radius, float progress, const Color& color, float thickness = 4, int num_segments = 64 ) {

	m_surface( )->DrawSetColor( color );

	if ( radius == 0.0f ) {
		m_surface( )->DrawFilledRect( center.x, center.y, center.x + 1, center.y + 1 );
		return;
	}
...
void render::CircularProgressBar_test(int x, int y, int radius, int start_angle, int percent, int points,  int thickness, Color color)
{
	float precision = (2 * 3.14159265358979) / 180;
	float step = 3.14159265358979 / 180;
	float inner = radius - thickness;
	...
	for (; radius > inner; --radius) {
		for (float angle = start_angle1337; angle < end_angle; angle += precision) {
			...
			m_surface()->DrawSetColor(color);
			m_surface()->DrawPolyLine(pointsx.data(), pointsy.data(), points);
		}
	}
}

void render::draw_arc2(int x, int y, int radius, int startangle, int percent, int thickness, Color color, int line_thinkness)
{
	float precision = (2 * pi) / 180;
	float step = pi / 180;
	float inner = radius - thickness;
	float end_angle = (startangle + percent) * step;
	float start_angle1337 = (startangle * pi) / 180;
