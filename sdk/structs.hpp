...
public:
    ...
    Vector* GetRenderAngleslocal()
    {
        static int iOffset = netvars::get().get_offset("DT_BasePlayer", "deadflag") + 0x4;
...
public:
    ...
    PNETVAR(UINT, m_hMyWearables, crypt_str("CBaseCombatCharacter"), crypt_str("m_hMyWearables"));
    ...
    NETVAR(int, m_iAddonBits, crypt_str("CBasePlayer"), crypt_str("m_iAddonBits"));
    ...
    TeamID GetTeam()
    {
        return *(TeamID*)((uintptr_t)this + m_iTeamNum());
    }
   ...
   // QAngle* GetVAngles();
   // bool CanSeePlayer(player_t* player, const Vector& pos);
    std::array<float, 24>& m_flPoseParameter_local();
   //int get_index();
   // void AttachmentHelper();
};
