...
bool weapon_t::can_double_tap_for_ind()
{
	if (!this) //-V704
		return false;
...
int weapon_t::get_max_tickbase_shift()
{
	...
	case WEAPON_G3SG1:
	case WEAPON_SCAR20:
		max_tickbase_shift = c_config::get()->i["rage_dtmode"] == 1 ? 14 : 12;
		break;
	}
...
std::array <float, 24>& player_t::m_flPoseParameter_local()
{
	static auto _m_flPoseParameter = netvars::get().get_offset(crypt_str("CCSPlayer"), crypt_str("m_flPoseParameter"));
	return *(std::array <float, 24>*)((uintptr_t)this + _m_flPoseParameter);
}
...
void player_t::AttachmentHelper()
{
	using fn = void(__thiscall*)(player_t*, CStudioHdr*);
	static fn attachement_helper = (fn)util::FindSignature(crypt_str("client.dll"), crypt_str("55 8B EC 83 EC 48 53 8B 5D 08 89 4D F4"));
...
void player_t::update_clientside_animation()
{
	...
	auto backup_m_bClientSideAnimation = m_bClientSideAnimation();
	g_ctx.globals.updating_animation = true;
	m_bClientSideAnimation() = true;
	using Fn = void(__thiscall*)(void*);
	call_virtual<Fn>(this, g_ctx.indexes.at(13))(this);
	this->m_bClientSideAnimation() = backup_m_bClientSideAnimation;
...
QAngle* player_t::GetVAngles()
{
	static auto deadflag = netvars::get().get_offset("CBasePlayer", "deadflag");
